...
local colliders = {}
local vect = API.load("vectr");
local blockdef = API.load("expandedDefines");
...
colliders.BLOCK_SEMISOLID = {8,25,26,27,28,38,69,79,108,121,122,123,130,161,168,240,241,242,243,244,245,259,260,261,287,288,289,290,372,373,374,375,379,380,381,382,389,391,392,437,438,439,440,441,442,443,444,445,446,447,448,506,507,508,568,572,575,579};
colliders.BLOCK_NONSOLID = {172,175,178,181};
...
colliders.BLOCK_PLAYER = {626,627,628,629,632,640,642,644,646,648,650,652,654,656,658,660,662,664};
]]
...
local function convertColor(h)
	return {math.floor(h/(256*256*256)),math.floor(h/(256*256))%256,math.floor(h/256)%256,h%256};
end
...
do -- Declare Box methods
	collidersTypeMetatables[TYPE_BOX].__index.Draw = function(obj, c)
		...
		color = convertColor(c), sceneCoords = true, primitive=Graphics.GL_TRIANGLE_FAN};
	end
end
...
do -- Declare Circle methods
	collidersTypeMetatables[TYPE_CIRCLE].__index.Draw = function(obj, c)
		...
		Graphics.glDraw{vertexCoords = pts, color = convertColor(co), sceneCoords = true};
	end
end
...
do -- Declare Tri methods
	...
	collidersTypeMetatables[TYPE_TRI].__index.Draw = function(obj, c)
		...
		color = convertColor(c), sceneCoords = true}
	end
end
...
function colliders.linecast(startPoint,endPoint,collider,dbg)
	...
	if(dbg == nil or not dbg) then
		...
		local ve = p or v2;
		Graphics.glDraw{vertexCoords = {v1.x,v1.y,ve.x,ve.y}, color={0,1,0,1}, sceneCoords = true, primitive = Graphics.GL_LINES}
		return b,p,n,o;
	end
end
...
function colliders.collideNPCBlock(a,b,sec,filter)
		...
		for _,v in pairs(NPC.get(a,sec)) do
			...
			if(hb ~= nil) then
				for _,v2 in pairs(Block.getIntersecting(hb.x,hb.y,hb.x+hb.width,hb.y+hb.height)) do
					if(contains(b,v2.id) and filter(v,v2) and colliders.collide(v,v2)) then
						c = c + 1;
	...
	elseif(idA and not idB) then
		local _,count,ps = colliders.collideNPC(b,a,sec,filter);
		c = count;
		cpairs = ps;
	elseif(not idA and idB) then
		local _,count,ps = colliders.collideBlock(a,b,sec,filter);
		c = count;
		cpairs = ps;
	else
		if(colliders.collide(a,b) and filter(a,b)) then c = 1; else c = 0; end;
	end
...
function colliders.collideBlock(a,b,sec,filter)
	...
	elseif(npcA and not npcB) then
		...
		if(hb ~= nil) then
			for _,v in pairs(Block.getIntersecting(hb.x,hb.y,hb.x+hb.width,hb.y+hb.height)) do
				if(contains(a,v.id) and filter(v) and colliders.collide(v,b)) then
					c = c + 1;
	...
	elseif(not npcA and npcB) then
		...
		if(hb ~= nil) then
			for _,v in pairs(Block.getIntersecting(hb.x,hb.y,hb.x+hb.width,hb.y+hb.height)) do
				if(contains(b,v.id) and filter(v) and colliders.collide(v,a)) then
					c = c + 1;
					table.insert(cpairs,v);
				end
			end
...
function colliders.collideNPC(a,b,sec,filter)
	...
	elseif(not npcA and npcB) then
		...
		if(colliders.collide(a,b) and filter(a,b)) then 
			c = 1;
		else 
			c = 0; 
		end;
